...
//Object.prototype.toString.call(val).slice(8,-1);获取val的数据类型
...
function toString(val) {
    return val == null ?
        '' :
        typeof val === 'object' ?
        JSON.stringify(val, null, 2) //将对象转化为json字符串，第二个参数用null，第三个参数是转化后的缩进
        :
        String(val)
}
...
var camelize = cached(function(str) {
    //string.replace(/-(\w)/g,function(a,b,c,d){})
    //当replace的第二个参数为函数时
    //该函数第一个参数是匹配该模式的字符串
    //该函数第二个参数是匹配改模式中某个圆括号子表达式的字符串，可能有0个或多个
    //该函数第三个参数是匹配结果在该字符串中的位置
    //该函数第四个参数是该字符串本身
...
/**
 * Hyphenate a camelCase string.将驼峰转为连字符
 */
var hyphenateRE = /\B([A-Z])/g; //\B边界匹配
